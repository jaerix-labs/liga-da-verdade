...
| 9 | Falta simples assinalada ou por assinalar | `falta_assinalada`; `falta_nao_assinalada` | Regista-se; impacto 0. Só entra aqui a falta **fora da área**, **sem cartão**, que **nenhum analista descreve como ocasião de golo travada** (senão é família 6). Ver D37 |
...
O JSON **não guarda "familia"**. Guarda `tipo` (um destes doze valores, fechado
...
`amarelo_nao_mostrado` | `falta_assinalada` | `falta_nao_assinalada`
...
| D37 | Novo par de `tipo` no vocabulário fechado: `falta_assinalada` / `falta_nao_assinalada` — família 9, impacto 0. Passa de 10 para 12 valores. Só entra aqui a decisão sobre falta **fora da área**, **sem cartão**, que **nenhum analista descreve como ocasião de golo travada** | Ao rever a jornada 3 (jogo em atraso Moreirense-Benfica), o lance 10' (Tomás Araújo rasteira Nile John junto à área, falta não assinalada, 3/3 "errado") não encaixava em nenhum tipo: não foi na área (não é penálti), não teve cartão (não é amarelo), e o João, lendo os recortes, concluiu que os analistas **não** descreviam uma ocasião de golo travada — logo não é família 6. Sem tipo próprio, a única saída era forçá-lo a `ocasiao_interrompida` escalão baixo (~0,02 de impacto, contra a leitura do João) ou deixá-lo de fora (contra a regra de entrada). O tipo novo dá-lhe casa: regista-se para o histórico e para o denominador das opiniões, com impacto 0 pela mesma razão dos amarelos (D12) — o efeito existe (posse, posição no campo), mas convertê-lo em pontos era inventar um número. **Guarda-costas contra abuso:** a `PROMPT-TRANSCRICAO.md` (v6.5) passa a exigir que toda a falta por assinalar diga à letra se travou ou não uma jogada de golo; só com "nenhum analista descreve ocasião" é que o lance pode vir para a família 9 |
